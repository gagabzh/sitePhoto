...
function renderRecipePills(query_json) {
  ...
  return pills.length
    ? pills.join(' ')
    : `<span style="font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--ink-faint)">(empty)</span>`;
}
...
function renderMyRecipesPage({ recipes, sharedWithMe, filtered, pinned, unpinned, search, view, isAdmin }) {
  ...
  return `<div class="tr-page">
    ...
    <div class="tr-filter">
      ...
      <div class="tr-seg">
        <a href="${viewUrl('cards', search)}" class="${view === 'cards' ? 'on' : ''}">cards</a>
        <a href="${viewUrl('list', search)}"  class="${view === 'list'  ? 'on' : ''}">list</a>
      </div>
    </div>
  ...
  <div id="tr-share-modal" class="tr-share-modal-backdrop" style="display:none">
    <div class="tr-share-modal">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-top:16px">
        ${isAdmin ? `<button id="tr-share-all" style="font-family:'Kalam',cursive;font-size:13px;padding:4px 12px;border:1.5px solid var(--ink);background:var(--ink);color:var(--paper);cursor:pointer;">📢 everyone</button>` : '<span></span>'}
...
module.exports = { renderRecipePills, recipeUrl, renderCard, renderRow, renderSharedCard, viewUrl, renderAdminPage, renderMyRecipesPage };
