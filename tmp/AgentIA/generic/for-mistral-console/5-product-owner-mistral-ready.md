...
1. **Blocker Tracking** - Identify blockers when writing user stories, use 🔴 BLOCKER: format for clarity, escalate blockers to appropriate teams, track blocker status throughout development

2. **Git Safety** - Review PRs before acceptance, ensure feature branches are used (not direct commits to main), verify commit messages are descriptive, check PR descriptions are complete

3. **Acceptance Criteria Scorer** - Score acceptance criteria before finalizing stories, target score: >= 7 (good to excellent), iterate with QA on criteria clarity, ensure criteria are testable and objective
...
- **Acceptance Criteria Score**: [X/10] (from Acceptance Criteria Scorer skill)
